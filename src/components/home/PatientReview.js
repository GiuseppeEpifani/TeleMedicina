...
const PatientReview = ({navigation, patient}) => {
    const createAlertDelete = () => {
		Alert.alert(
			"¿Esta seguro?",
			`Eliminara al paciente: ${patient.run}` ,
			[
				{
					text: "Cancelar",
            ...
            <View style={{flex: 1.3}}>
                <View style={{flex: 0.2, justifyContent: 'center'}}>
                    <View style={{flexDirection: 'row', justifyContent: 'center'}}>
                        ...
                        />
                        <Text style={styles.titlePatient}>{patient.name}</Text>      
                    </View>
                </View>
                <View style={styles.containerInfoPatient}>
                    <ScrollView>
                        <TextIcon label={"RUN"} text={patient.run} icon={"card-account-details"}/>
                        <TextIcon label={"Género"} text={'Masculino'} icon={"gender-male-female"}/>
                        <TextIcon label={"Fecha de nacimiento"} text={'21-07-1998'} icon={"calendar-range"}/>
                        <TextIcon label={"Enfermedad"} text={'Diabetes'} icon={"virus"}/>
